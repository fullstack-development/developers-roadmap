...
* Свойство `length` у массива:
    ...
    * Влияет ли на `length` удаление элемента посередине массива? (метод delete)
* Как проверить, что в переменной лежит массив? Возможны как минимум 3 способа.
* Как работает метод reduce у массивов? Какие нюансы работы с этим методом, какие есть другие методы по работе с массивами и где лучше применять reduce, а где что-нибудь другое? Описать методы sort, filter, map, reduce, forEach, some, every. Описать подробно какие параметры принимают, какие параметры принимают функции callback, что возвращают и т.д.

#### Ресурсы
* [Массивы. MDN](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Array)

## Функции
* 4 шаблона вызова функции, которые задают контекст выполнения этой функции. Расказать о методах call/apply и в чем их отличие. Предложить свою реализацию оператора new своей функцией (вопрос на предмет понимания работы функции apply).
